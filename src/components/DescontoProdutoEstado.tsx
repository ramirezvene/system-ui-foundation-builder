...
export default function DescontoProdutoEstado() {
  ...
  const fetchData = async () => {
    try {
      ...
      const {
        data: margemData,
        error: margemError
      } = await supabase.from("produto_margem").select("*").eq("tipo_aplicacao", "estado").order("id");
      ...
      const {
        data: estadosData,
        error: estadosError
      } = await supabase.from("cadastro_estado").select("*").eq("st_ativo", 1).order("estado");
  ...
  const handleExportCSV = () => {
    const csvContent = [["ID", "ID Produto", "Nome Produto", "Estado", "Tipo Margem", "Margem", "Margem Adicional", "% Desconto", "Data Início", "Data Fim", "Ativo", "Observação"], ...produtoMargens.map(item => [item.id, item.id_produto, item.produto?.nome_produto || "", item.tipo_referencia || "", item.tipo_margem, item.margem, item.margem_adc || "", item.desconto || "", item.data_inicio, item.data_fim, item.st_ativo === 1 ? "Ativo" : "Inativo", item.observacao || ""])].map(row => row.join(",")).join("\n");
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="border-b bg-gray-50">
                ...
                <th className="text-left p-3 min-w-[50px] text-sm font-medium px-[12px]">Tipo Margem</th>
            ...
            <tbody>
              {filteredProdutoMargens.map(item => <tr key={item.id} className={`border-b hover:bg-gray-50 ${editedRows.has(item.id) ? 'bg-yellow-50' : ''}`}>
                  ...
                  <td className="p-3">
                    <EstadoCombobox estados={estados} selectedEstado={getSelectedEstado(item)} onEstadoChange={estado => handleTipoReferenciaChange(item.id, estado)} disabled={item.st_ativo === 0} />
                  </td>
                   <td className="p-3">
                  ...
                  <td className="p-3">
                   ...
                   <td className="p-3">
                     <Switch checked={item.st_ativo === 1} onCheckedChange={checked => handleFieldChange(item.id, 'st_ativo', checked ? 1 : 0)} disabled={false} />
                   </td>
                  <td className="p-3">
